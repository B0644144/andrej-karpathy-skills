...
一個單一的 `CLAUDE.md` 文件，用於改善 Claude Code 的行為，源自 [Andrej Karpathy 的觀察](https://x.com/karpathy/status/2015883857489522876) 關於 LLM 編碼陷阱的總結。
...
> "它們真的很喜歡把程式碼和 API 搞複雜，堆砌抽象概念，不清理死程式碼……明明 100 行能搞定的事情，非要實作現成 1000 行的臃腫架構。"

> "它們有時仍會改動或刪除自己理解不足的程式碼和注釋，即使這些內容與任務本身無關。"

## 解決方案

四個原則，集中在一個文件中，直接解決這些問題：
...
強有力的成功標準讓 LLM 能夠獨立循環執行。弱標準（"讓它工作"）需要不斷澄清。
...
這會將指南安裝為 Claude Code 插件，使其在您所有專案中可用。
...
本倉庫包含一個已提交的 Cursor 專案規則 ([`.cursor/rules/karpathy-guidelines.mdc`](.cursor/rules/karpathy-guidelines.mdc))，因此在 Cursor 中打開專案時同樣適用這些指南。詳情請參見 **[CURSOR.md](CURSOR.md)**，包括如何在其他專案中使用該規則，以及它與 Claude Code 的關係。
...
"目標驅動執行"原則正是捕捉了這一點：將指令式指令轉化為帶有驗證循環的聲明式目標。

## 如何判斷它在起作用

如果您看到以下情況，說明這些指南正在發揮作用：

- **diff 中不必要的改動更少** —— 只有請求的改動出現
- **因過度複雜而導致的重寫更少** —— 程式碼第一次就寫得簡潔
- **澄清問題在實作之前提出** —— 而不是在犯錯之後
- **乾淨、精簡的 PR** —— 沒有順帶的重構或"改進"

## 客製化

這些指南設計用於與專案特定指令合併。將它們添加到您現有的 `CLAUDE.md` 或建立一個新的。
